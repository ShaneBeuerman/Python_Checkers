...
while True:
    ...
    if xStart is not int or yStart is not int or int(xStart) > 8 or int(yStart) > 8 or int(xStart) < 0 or int(yStart) < 0:
        print("Sorry but that input doesn't work")
        continue
    x = input("Choose an x position.")
    y = input("Choose a y position")
    if x is not int or y is not int or int(x) > 8 or int(y) > 8 or int(x) < 0 or int(y) < 0 or x is not int or y is not int:
        print("Sorry but that input doesn't work")
        continue
    # Move Piece
